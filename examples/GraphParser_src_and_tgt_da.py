...
logger = get_logger('GraphParser_joist_rc_tgt')

def read_arguments():
    args_ = argparse.ArgumentParser(description='Sovling GraphParser_joist_rc_tgt')
    ...
    args_.add_argument('--freeze', action='store_true', help='frozen the word embedding (disable fine-tuning).')
    ...
    args_.add_argument('--load_sequence_tagger_paths', nargs='+', help='path for loading saved sequence_tagger saved_models files.', default=None)
    ...
    for split in args_dict['splits']:
        ...
        if split == 'train':
            args_dict['data_paths'][split] = [data_path + '_' + split + '_' + args_dict['src_domain'],
                                              'saved_models/ontonotes_parser_' + args_dict['src_domain'] + '_full_train/domain_'+args_dict['tgt_domain']+'_'+split+'_model_domain_'+args_dict['tgt_domain'] + '_data_domain_'+args_dict['tgt_domain'] +'_pred.txt']
        elif split == 'dev':
            args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['src_domain']
        else:
            args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['tgt_domain']

        # # For DA no sequence_tagger
        # if split != 'test':
        #     args_dict['data_paths'][split] = 'saved_models/ontonotes_parser_nw_da/' \
        #                               'domain_nw_tasks_dp_' + split + '_model_domain' \
        #                               '_nw_data_domain_' + args_dict['domain'] + '_pred.txt'
        # else:
        #     args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['domain']
        #
        # ## FOR DA after sequence_tagger
        # if split != 'test':
        #     args_dict['data_paths'][split] = data_path + '_' + split + '_' + 'nw'
        # else:
        #     args_dict['data_paths'][split] = data_path + '_' + split + '_' + args_dict['domain']
    ...
    args_dict['load_sequence_tagger_paths'] = args.load_sequence_tagger_paths
    if args_dict['load_sequence_tagger_paths'] is not None:
        args_dict['gating'] = True
        args_dict['num_gates'] = len(args_dict['load_sequence_tagger_paths']) + 1
    else:
    ...
    args_dict['word_status'] = 'frozen' if args.freeze else 'fine tune'
...
def construct_embedding_table(alphabet, tokens_dict, dim, freeze, token_type='word'):
    if tokens_dict is None:
        return None
    scale = np.sqrt(3.0 / dim)
    table = np.empty([alphabet.size(), dim], dtype=np.float32)
    table[prepare_data.UNK_ID, :] = np.zeros([1, dim]).astype(
        np.float32) if freeze else np.random.uniform(-scale, scale, [1, dim]).astype(np.float32)
    oov_tokens = 0
    for token, index in alphabet.items():
        if token in tokens_dict:
            ...
            embedding = np.zeros([1, dim]).astype(np.float32) \
                if freeze else np.random.uniform(-scale, scale, [1, dim]).astype(np.float32)
            oov_tokens += 1
...
#         gpu_output = run(["nvidia-smi", "-q", "-d", "PIDS"], capture_output=True, text=True)
...
def build_model_and_optimizer(args):
    word_table = construct_embedding_table(args.alphabets['word_alphabet'], args.word_dict, args.word_dim, args.freeze, token_type='word')
    char_table = construct_embedding_table(args.alphabets['char_alphabet'], args.char_dict, args.char_dim, args.freeze, token_type='char')
    pos_table = construct_embedding_table(args.alphabets['pos_alphabet'], args.pos_dict, args.pos_dim, args.freeze, token_type='pos')
    ...
    if args.load_sequence_tagger_paths:
        pretrained_dict = {}
        model_dict = model.state_dict()
        for idx, path in enumerate(args.load_sequence_tagger_paths):
...
def in_domain_evaluation(args, datasets, model, optimizer, dev_eval_dict, test_eval_dict, epoch,
    ...
    curr_dev_eval_dict = evaluation(args, datasets['dev'], 'dev', model, args.src_domain + '_' + args.tgt_domain, epoch, 'current_results')
    ...
    if is_best_in_domain:
        ...
        curr_test_eval_dict = evaluation(args, datasets['test'], 'test', model, args.src_domain + '_' + args.tgt_domain, epoch, 'current_results')
    ...
    if epoch == args.num_epochs:
        ...
        for split in splits_to_write:
            if split == 'dev':
                ...
                eval_dict = None
            write_results(args, datasets[split], args.src_domain + '_' + args.tgt_domain, split, best_model, dom, eval_dict)
...
def main():
    ...
    logger.info('Training INFO of in domain %s' % (args.src_domain + '_' + args.tgt_domain))
    ...
    logger.info('number of training samples for %s is: %d' % (args.src_domain + '_' + args.tgt_domain, num_data))
    ...
    if not args.eval_mode:
        ...
        for epoch in range(start_epoch + 1, args.num_epochs + 1):
            ...
            for batch_num, batch in enumerate(train_iter):
                ...
                if batch_num % 500 == 0:
                    log_info = 'train: %d/%d, domain: %s, total loss: %.2f, arc_loss: %.2f, arc_tag_loss: %.2f, time left: %.2fs' % \
                               (batch_num, num_batches, args.src_domain + '_' + args.tgt_domain, total_loss / total_train_inst, total_arc_loss / total_train_inst,
            ...
            if patient >= args.schedule:
                ...
                patient = 0
            print_results(test_eval_dict['in_domain'], 'test', args.src_domain + '_' + args.tgt_domain, 'best_results')
            print('\n')
        for split in datasets.keys():
            evaluation(args, datasets[split], split, best_model, args.src_domain + '_' + args.tgt_domain, epoch, 'best_results')
