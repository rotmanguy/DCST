...
from utils.models.parsing import BiAffine_Parser
...
logger = get_logger('GraphParser_ST')

def read_arguments():
    args_ = argparse.ArgumentParser(description='Sovling GraphParser_ST')
    args_.add_argument('--dataset', choices=['ontonotes', 'ud'], help='Dataset', required=True)
    #args_.add_argument('--domain', choices=['all', 'all_without_pt', 'bc', 'bn', 'mz', 'nw', 'pt', 'tc', 'wb'],
    ...
    args_.add_argument('--parser_path', help='path for loading parser files.', required=True)
    ...
    for split in args_dict['splits']:
        if split == 'train':
            args_dict['data_paths'][split] = [args_dict['parser_path'] + file for file in os.listdir(args_dict['parser_path']) if (file.endswith("pred.txt") and 'extra' in file) or (file.endswith("gold.txt") and 'train' in file and 'extra' not in file)]
        else:
            args_dict['data_paths'][split] = [args_dict['parser_path'] + file for file in os.listdir(args_dict['parser_path']) if (file.endswith("gold.txt") and split in file and 'extra' not in file)]
    args_dict['alphabet_data_paths'] = {}
    for split in args_dict['splits']:
        if args_dict['dataset'] == 'ontonotes':
            args_dict['alphabet_data_paths'][split] = data_path + '_' + split + '_' + 'all'
        else:
            args_dict['alphabet_data_paths'][split] = data_path + '_' + split + '_' + args_dict['domain']
...
def build_model_and_optimizer(args):
    ...
    model = BiAffine_Parser(args.word_dim, args.num_word, args.char_dim, args.num_char,
                            ...
                            args.arc_space, args.arc_tag_space,
